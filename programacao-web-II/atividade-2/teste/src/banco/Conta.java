...
abstract public class Conta {
    ...
    public void investir(double valor) {
        ...
        double saldoDisponivel = this.verSaldo() + this.limite;

        if (valor > saldoDisponivel) throw new IllegalStateException();

        // usa saldo primeiro
        if (valor <= this.verSaldo()) {
            this.sacar(valor);
        } else {
            double restante = valor - this.verSaldo();

            // zera saldo
            this.sacar(this.verSaldo());
